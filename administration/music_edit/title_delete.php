...
<div id="main">
	<div id="pagetitle">練習日設定</div>
		...
		foreach( $_POST['id'] as $id ){
			$musicName = $db->executeQuery("SELECT name FROM music_titles WHERE id='{$id}'");
			$db->executeQuery("DELETE FROM music_titles WHERE id='{$id}';");
			$db->executeQuery("DELETE FROM music_urls WHERE title_id='{$id}';");
